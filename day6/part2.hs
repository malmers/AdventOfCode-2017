...
stripList :: [[Int]] -> [Int] -> Int
stripList (x:xs) elem | x == elem = length (x:xs)
                      | otherwise = stripList xs elem

blockcycle :: [[Int]] -> Int -> Int
blockcycle blocks cycles | any (\x -> x == nextBlock) blocks = stripList blocks nextBlock
